...
| Product / version | Landsat Collection 2 Level-2 surface reflectance is the documented product option; series collection still to be linked |
| Geographic coverage | Global scene archive |
| Available time coverage | Landsat 4–9 surface-reflectance archive from 1982 onward; mission/scene availability varies |
| Project time coverage | Series-specific dates not recorded |
| Temporal interval | Scene acquisitions; record actual dates and composite window |
| Native spatial resolution / scale | 30 m surface reflectance; other Landsat products/bands differ |
...
An export grid does not establish sensor resolution. Record sensor, QA mask, scaling and scene count after cloud filtering.

[Provider specification](https://www.usgs.gov/landsat-missions/landsat-surface-reflectance)
...
[Original discovery link](https://landsat.gsfc.nasa.gov/data/). Choose the specific collection or layer and document a bounded download, subset query, or manual request before using it in a series.
